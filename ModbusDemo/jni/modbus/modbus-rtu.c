...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
    ...
    int r = read(ctx->s, rsp, rsp_length);
#if 0
    for (i = 0; i < rsp_length; ++i) {
    	snprintf(buf + i * 3, 4, "%02x ", rsp[i]);
    }
    LOGD("recv: %s", buf);
#endif
    return r;
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    default:
        dcb.BaudRate = CBR_9600;
        if (ctx->debug) {
            fprintf(stderr, "WARNING Unknown baud rate %d for %s (termbits.h used)\n",
                              ...
                              struct timeval *tv, int length_to_read)
{
    ...
    if (s_rc == 0) {
        /* Timeout */
    	fprintf(stderr, "select ETIMEDOUT %d\n",errno);
        errno = ETIMEDOUT;
        return -1;
    }
#endif

	fprintf(stderr, "select ok %d\n",errno);
    return s_rc;
}
